...
import {FrontpageSlicesHome_Hero_Slice, PageSlicesImage} from "../../prismic-types";
...
function Plane(props) {
  ...
  const vertexProps = useControls('displacement', {
    amplitude: {value: 0.50, min: 0, max: 0.5, step: 0.01},
    frequency: {value: 5.83, min: 1, max: 10},
    speed: {value: 3.6, min: 1, max: 20}
  })

  const fragmentProps = useControls('effect', {
    zoom: {value: 3.0, min: 0.01, max: 5},
    twist: {value: 0.20, min: -1, max: 1},
    ...
    <>
      ...
      {showTexture && (
        <Html fullscreen style={{pointerEvents: 'none'}}>
          <img className="debugTexture"
               src={matProps.debug ? DEBUG_TEXTURE : CAUSTICS_TEXTURE} alt="debug"/>
        </Html>
      )}
    </>
  )
